...
public:
  ...
  void BrushMove(const Point source, const Point target,
    ...
    gl_draw_shape(GL_POINTS, [&] {
      ...
      auto pixel =
          pDoc->m_pUI->m_origView->original_img(valid_source.y, valid_source.x);
      color[0] = get<0>(pixel);
      color[1] = get<1>(pixel);
      color[2] = get<2>(pixel);
      ...
      for (int i = -half_width; i < half_width; ++i) {
        ...
        for (int j = -half_height; j < half_height; ++j) {
          ...
          alphaind_y++;
          if (alphaind_y >= pDoc->alpha_image.height)
            break;
          glColor4ubv(color);
          gl_set_point(cur);
        }
        alphaind_x++;
        if (alphaind_x >= pDoc->alpha_image.width)
          break;
      }
    });
  ...
  GLubyte getAlpha(const Point source) {
    ...
    GLubyte alpha =
        get<0>(pixel) * 0.3 + get<1>(pixel) * 0.6 + get<2>(pixel) * 0.1;
    return alpha;
  }
  void select() {
    ImpressionistDoc *pDoc = GetDocument();
