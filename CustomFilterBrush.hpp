...
public:
  ...
  void SetColor(const Point source) {
    ...
    ImageUtils::filterCustom(pDoc->m_pUI->m_origView->original_img, source,
                             int_color, filter_vals, row_num, col_num, divisor);
    color[0] = int_color[0];
    color[1] = int_color[1];
    color[2] = int_color[2];
  ...
  bool checkValidFilter() const { return (row_num > 0 && col_num > 0); }
